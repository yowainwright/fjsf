...
import { getRunCommand, getWorkspaceRunCommand } from '../src/package-manager.ts';
...
describe('getWorkspaceRunCommand', () => {
  it('returns correct workspace command for npm', () => {
    expect(getWorkspaceRunCommand('npm', 'my-package')).toBe('npm run --workspace my-package');
  });

  it('returns correct workspace command for pnpm', () => {
    expect(getWorkspaceRunCommand('pnpm', 'my-package')).toBe('pnpm --filter my-package run');
  });

  it('returns correct workspace command for yarn', () => {
    expect(getWorkspaceRunCommand('yarn', 'my-package')).toBe('yarn workspace my-package');
  });

  it('returns correct workspace command for bun', () => {
    expect(getWorkspaceRunCommand('bun', 'my-package')).toBe('bun run --filter my-package');
  });
});
