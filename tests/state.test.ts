...
import { createInitialState, updateSelection, getSelectedScript } from '../src/state.ts';
...
describe('state', () => {
  const mockScripts: PackageScript[] = [
    ...
    { name: 'dev', command: 'bun dev', workspace: 'root', packagePath: 'package.json' },
  ];
  ...
  describe('getSelectedScript', () => {
    it('returns selected script', () => {
      ...
      expect(selected).toEqual(mockScripts[0]);
    });

    it('returns null when no matches', () => {
      const state = { ...createInitialState([]), matches: [] };
      const selected = getSelectedScript(state);
